...
export default function OperatingModel() {
  ...
  return (
    // التغيير 1: الخلفية bg-zinc-100 (رمادي فضي) بدلاً من الأبيض
    <section className="relative h-screen bg-zinc-100 overflow-hidden flex flex-col justify-center py-8 font-sans text-zinc-900">
      
      {/* Background Decor (Darker Grid lines for contrast) */}
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10 w-full flex flex-col h-full justify-between max-h-[90vh]">
          ...
          {phases.map((phase, i) => (
            ...
            >
              ...
              <div className="bg-white border border-zinc-300 hover:border-amber-600 hover:shadow-2xl hover:shadow-zinc-400/20 transition-all duration-500 p-6 flex flex-col h-full rounded-lg relative overflow-hidden group-hover:-translate-y-2">
                ...
                {/* Step Number */}
                <div className="absolute top-2 right-4 text-6xl font-black text-zinc-100 opacity-50 group-hover:text-amber-50 transition-colors pointer-events-none">
                  {phase.number}
                </div>

                {/* Icon */}
                <div className="w-12 h-12 shrink-0 bg-zinc-50 border border-zinc-200 text-zinc-500 group-hover:bg-amber-600 group-hover:text-white group-hover:border-amber-600 rounded-lg flex items-center justify-center mb-5 transition-all duration-300 shadow-sm">
                  <phase.icon size={20} />
                </div>

                {/* Title */}
                <div className="mb-3 relative z-10">
                  <div className="text-[9px] font-bold uppercase text-amber-700 tracking-wider mb-1">{phase.subtitle}</div>
                  <h3 className="text-xl font-black text-zinc-900">{phase.title}</h3>
                </div>

                {/* Description */}
                <p className="text-zinc-500 text-[11px] font-bold leading-relaxed mb-6 line-clamp-3 relative z-10">
                  {phase.description}
                </p>

                {/* Deliverables Box (Grey inside White) */}
                <div className="mt-auto bg-zinc-100 rounded border border-zinc-200 p-3 group-hover:border-amber-100 transition-colors">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="pb-6 pt-4 border-t border-zinc-300 flex flex-col md:flex-row items-center justify-between gap-6"
        >
          {/* Stats */}
          <div className="flex gap-8 lg:gap-16">
            <div>
              <div className="text-2xl font-black text-zinc-900">100%</div>
              <div className="text-[9px] uppercase tracking-widest text-zinc-500 font-bold">Completion Rate</div>
            </div>
            <div>
              <div className="text-2xl font-black text-zinc-900">4-12</div>
              <div className="text-[9px] uppercase tracking-widest text-zinc-500 font-bold">Weeks Avg. Timeline</div>
            </div>
          </div>

          {/* CTA Button */}
          <Link to="/operating-model"> 
            <button className="group flex items-center gap-3 px-6 py-3 bg-zinc-900 hover:bg-amber-600 text-white text-[10px] font-black uppercase tracking-widest transition-all rounded-sm shadow-xl shadow-zinc-400/20 hover:shadow-amber-200">
