...
interface CaseStudy {
  ...
  bgBase: string;      // NEW: Background color for the whole section
...
export default function CaseStudiesProFinal() {
  ...
  const cases: CaseStudy[] = [
    {
      ...
      // لون الخلفية: كحلي غامق جداً (Dark Slate)
      bgBase: "bg-[#0f172a]", 
    ...
    },
    {
      ...
      // لون الخلفية: أسود دافئ (Warm Stone)
      bgBase: "bg-[#1c1917]", 
    ...
    },
    {
      ...
      // لون الخلفية: أخضر غامق جداً (Deep Teal)
      bgBase: "bg-[#022c22]", 
  ...
  return (
    // Added transition-colors duration-1000 for smooth background change
    <section className={`relative h-screen overflow-hidden flex flex-col justify-center py-8 transition-colors duration-1000 ease-in-out ${currentCase.bgBase}`}>
      
      {/* Background Texture (Static to prevent flicker) */}
      <div className="absolute inset-0 opacity-[0.03] pointer-events-none bg-[url('https://www.transparenttextures.com/patterns/stardust.png')]"></div>

      <div className="max-w-7xl mx-auto px-8 relative z-10 w-full flex flex-col h-full max-h-[90vh] justify-between">
        
        {/* --- HEADER --- */}
        <div className="flex flex-col md:flex-row justify-between items-end border-b border-white/5 pb-6 mb-4 gap-6">
          ...
          <div>
            ...
            >
              <Sparkles size={14} className={currentCase.accent} />
              <span className={`font-bold text-[10px] tracking-[0.4em] uppercase ${currentCase.accent}`}>Success Stories</span>
            </motion.div>
          ...
          <div className="flex items-center gap-8 pb-2">
              ...
              >
                <div className="flex flex-col items-end">
                    <span className="text-[9px] text-zinc-500 uppercase font-bold tracking-widest group-hover:text-zinc-300 transition-colors">Next</span>
                    <span className="text-xs font-bold text-white transition-colors">
                        {nextCaseData.client}
                    </span>
                </div>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center bg-white text-black shadow-lg shadow-white/10 transition-transform duration-300 group-hover:scale-110`}>
        ...
        {/* --- MAIN CONTENT --- */}
        <div className="grid md:grid-cols-2 gap-8 lg:gap-16 items-center flex-1 min-h-0">
          ...
          <div className="relative h-full w-full flex items-center justify-center p-2">
             <AnimatePresence mode="wait">
                <motion.div 
                  ...
                  className={`relative w-full h-[320px] md:h-[420px] rounded-2xl overflow-hidden shadow-2xl ${currentCase.glow} border ${currentCase.border} group cursor-pointer`}
                >
                  <img 
                  ...
                  {/* Floating Pill */}
                  <div className="absolute top-4 left-4 bg-black/40 backdrop-blur-md border border-white/10 px-3 py-1.5 rounded-full flex items-center gap-2 shadow-lg z-10">
                     <div className={`w-1.5 h-1.5 rounded-full ${currentCase.bgAccent}`}></div>
                     <span className="text-[9px] font-bold text-white tracking-widest uppercase">{currentCase.industry}</span>
                  </div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <div className="space-y-6">
                  <div>
                    <h3 className="text-3xl font-black text-white mb-3 leading-tight">{currentCase.client}</h3>
                    <p className="text-zinc-400 text-sm leading-relaxed border-l-2 border-white/10 pl-4">
                      {currentCase.solution}
                    </p>
                  </div>

                  {/* Results List - Hover Effect Added */}
                  <div className="space-y-3">
                    {currentCase.results.map((result, i) => (
                      <div 
                        key={i} 
                        className={`flex items-center justify-between p-4 rounded-lg border border-white/5 bg-white/[0.02] transition-all duration-300 group cursor-default ${currentCase.statHover}`}
                      >
                        <div className="flex items-center gap-3">
                          <div className={`p-2 rounded-full bg-white/5 ${currentCase.accent} group-hover:bg-white/10 transition-colors`}>
                            <result.icon size={16} />
                          </div>
                          <span className="text-[10px] text-zinc-500 uppercase font-bold tracking-wider group-hover:text-zinc-300 transition-colors">{result.label}</span>
                        </div>
                        <span className="text-xl font-bold text-white font-mono">{result.value}</span>
                      </div>
                    ))}
                  </div>
                  
                  {/* Dynamic Color CTA Button */}
                  <motion.button
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: 0.2, duration: 0.4 }}
                    className={`mt-2 w-full group flex items-center justify-center gap-3 p-4 rounded-lg border border-white/10 bg-white/5 transition-all duration-300 ${currentCase.buttonHover}`}
                  >
                    <span className="text-xs font-black uppercase tracking-widest text-white group-hover:text-black transition-colors">
                        Read Full Case Study
                    </span>
                    <ArrowRight size={16} className="text-white group-hover:text-black transition-colors group-hover:translate-x-1" />
                  </motion.button>
